...
/**
 * Domínios de links encurtados da Amazon conhecidos
 */
 ...
 */
export function isShortUrlDomain(hostname: string): boolean {
	const lowerHostname = hostname.toLowerCase();
...
/**
 * Valida se uma URL é da Amazon ou um encurtador conhecido (validação rápida sem resolver)
 * Útil para validação em tempo real sem fazer requisições HTTP
 * 
 * @param url - URL a ser validada
 * @returns Objeto com resultado da validação
 */
export function validateAmazonUrl(url: string): { valid: boolean; message?: string } {
	try {
		const urlObj = new URL(url);
		const hostname = urlObj.hostname.toLowerCase();

		// Verifica se é Amazon ou encurtador
		const isValid = isAmazonDomain(hostname) || isShortUrlDomain(hostname);

		if (!isValid) {
			return {
				valid: false,
				message: 'URL deve ser da Amazon ou um link encurtado válido (amzn.to, a.co, amzlink.to)'
			};
		}
		...
		return {
			valid: false,
			message: 'URL inválida. Por favor, informe uma URL completa.'
		};
	}
}
