...
import { createTestAction } from '../../../test-helpers/factories';
...
describe('AddProductActionProcessor - New Fields', () => {
	...
	it('should create product with all new fields (category, format, genre, publisher, contributors, productGroup, store)', async () => {
		// Arrange
		const amazonProduct = {
			offerId: 'AMAZON',
	...
	it('should create product with undefined new fields when not provided by Amazon API', async () => {
		...
		const action = createTestAction('B012345679', {
			id: 'action-1',
			user_id: 'user-1',
			is_processed: 0
		});
